...
Currently, this seems to work for v1.10. **WARNING: If inputting names into Spec II's improved keyboard, disable HD HUD / UI textures, as it renders some symbols invisible on the keyboard (e.g. what looks like `BOLT STORY` might actually be `BOLT_STORY`).**
...
- `SpecII-Fairness-Tester`: A debug tester tool to show the fairness of car distribution across all possible lineups.
